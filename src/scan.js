...
export async function scan(url, { allRules = false } = {}) {
  ...
  try {
    ...
    for (const v of results.violations) {
      for (const n of v.nodes) {
        ...
        out.push({
          ruleId: v.id, impact: v.impact, wcag: v.tags.filter((t) => /^wcag\d/.test(t)),
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (!urls.length) { console.error('Usage: node src/scan.js <url> [<url> ...] [--all-rules]'); process.exit(1); }
  const all = [], seen = new Set();
  for (const u of urls) {
    const found = await scan(u, { allRules });
